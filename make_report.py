...
body("[학습 과정 해석] 그림 4의 MAPE 곡선은 학습이 진행되는 동안 모델의 거동을 단계적으로 보여 준다. "
     ...
     "더 이상 개선되지 않음을 보여 준다. ④(EarlyStopping의 역할) 검증 오차가 오랫동안 개선되지 않자 epoch "
     "1011에서 학습이 자동 종료되었고, restore_best_weights 설정에 따라 최종 모델은 과적합된 후반 상태가 "
     "아니라 ②의 최적 시점(epoch 11) 가중치로 복원되었다. 그 결과 테스트 성능이 MAPE 1.20%로 우수하게 "
     "유지될 수 있었다.")

subheading("2) 예측 성능 평가 및 예측 정확도 분석 (Test data)")
body("최적 모델(epoch 11의 복원된 가중치)을 학습에 사용하지 않은 테스트 데이터에 적용하여 예측 성능을 "
     "정량적으로 평가하였다. 결과는 다음과 같다.")
make_table(["지표", "MAPE", "MAE", "RMSE"],
           [["값", "1.20 %", "≈ 1.8 GPa", "≈ 2.6 GPa"]])
one_image(os.path.join(DS, "E_reference_vs_prediction.png"),
          "그림 5. 실제값(Reference) vs 예측값(Prediction) — Target: E", w=8.2)
body("[성능 해석] 탄성계수의 분포 범위가 60~240 GPa임을 고려하면, MAPE 1.20%와 MAE 약 1.8 GPa는 매우 "
     "높은 예측 정확도에 해당한다. RMSE(≈2.6 GPa)가 MAE보다 다소 큰 것은 일부 표본에서 상대적으로 큰 오차가 "
