...
body("[결과 해석] 응력집중은 하중 방향과 수직인 홀의 좌·우 측면(x=cx±r, y=0)에서 가장 크게 나타났고, 하중 "
     ...
     "거동이다. 기본 조건(r=0.15)에서 홀 가장자리의 최대 σyy는 약 54 MPa로, 구멍에서 충분히 떨어진 공칭응력"
     "(약 19.6 MPa)의 약 2.75배에 해당하는 응력집중이 발생하였다. 한편 본 해석은 변위를 강제하는 조건이므로, "
...
body("선택 변수로 구멍 크기 r을 0.05~0.30으로 변화시키며 응력집중계수(SCF=σyy,max/σ_nominal)를 분석하였다. "
     "응력 구배가 급격한 홀 가장자리에서의 정확도를 높이기 위해 홀 경계 주변 메쉬를 점진적으로 조밀화하였으며, "
     "공칭응력은 gross 단면(폭 W)과 net 단면(폭 W−2r) 두 기준으로 각각 계산하였다. 결과는 다음과 같다.")
...
body("[결과 해석] 첫째, 가장 작은 구멍(r=0.05)에서 SCF(gross) ≈ 2.91 로, 무한 평판·소형 원공의 고전 Kirsch "
     "이론값 3에 거의 수렴하였다. 이는 본 유한요소 모델이 물리적으로 타당하게 작동함을 검증해 주는 핵심 결과다. "
     "둘째, 구멍이 커질수록 첨두 응력 σyy,max 자체는 62→43 MPa로 감소하였는데, 이는 변위 구속 조건에서 구멍이 "
     "커질수록 판이 유연해져 전달되는 하중(반력)이 줄기 때문이다. 셋째, net 단면 기준 SCF는 2.76→1.86으로 "
     "단조 감소하였다. 이는 구멍이 커질수록 단면 응력 분포가 상대적으로 균등해지는 효과이며, 동시에 본 시편은 "
     "높이 H가 짧고 구멍이 상·하 경계에 근접해 무한 스트립 가정에서 벗어나는 유한 효과가 반영된 것이다. "
     "(유한 폭 평판에서는 보정계수로 인해 d/W 증가 시 gross 기준 SCF가 커지는 경향이 알려져 있으나, 짧은 "
     "시편·변위구속이라는 본 조건에서는 직접 비교 시 주의가 필요하다.)")
body("[해석상 고려사항] 본 해석에 사용된 CST(상수변형률 삼각형) 요소는 요소당 응력이 일정해 응력 구배가 큰 홀 "
     "가장자리에서 첨두 응력을 과소평가하는 경향이 있다. 실제로 홀 주변 메쉬를 조밀화하자 SCF가 약 2.5에서 "
     "2.9로 증가하여 이론값 3에 더 근접하였다. 더 높은 정확도가 필요하다면 2차(quadratic) 요소 도입이나 추가 "
     "메쉬 세분화가 효과적일 것으로 판단된다.")
...
bullet("정규화와 일반화: 입력 변수의 스케일 차이를 Min-Max 정규화로 통일하여 학습 안정성을 확보하였고, Dropout(0.2)·L2 정규화·EarlyStopping을 통해 과적합을 억제하였다. Training MAPE와 Validation MAPE의 격차가 크지 않아 모델이 안정적으로 일반화되었음을 확인하였다.")
