...
body("[학습 과정 해석] 그림 4의 MAPE 곡선은 학습이 진행되는 동안 모델의 거동을 단계적으로 보여 준다. "
     "①(초기 급감, 약 0~10 epoch) Training MAPE는 약 55%, Validation MAPE는 약 21%에서 출발하여 불과 "
...
body("[방법] 학습된 모델이 12개 입력 변수 중 무엇에 의존하여 탄성계수를 예측하는지 확인하기 위해 두 가지 해석 "
     "기법을 적용하였다. Permutation Importance는 특정 변수의 값만 무작위로 섞은 뒤 예측 오차(MAPE)가 얼마나 "
     "증가하는지를 측정하는 방법으로 증가량이 클수록 중요한 변수이며, SHAP은 게임이론의 Shapley 값을 이용해 각 "
     "예측에 대한 변수별 기여도를 정량화한다. 원리가 서로 다른 두 방법을 함께 사용해 결과를 교차 검증하였다.")
...
body("[결과 해석] 그림 6·7에서 보듯 두 방법 모두 Stiffness2(제하 강성, unloading stiffness)를 가장 중요한 "
     "변수로 일관되게 지목하였다. 그 영향력은 압도적이어서, Permutation Importance에서는 이 변수를 무작위로 "
     "섞었을 때 테스트 MAPE가 약 +50%p나 폭증한 반면 나머지 변수들은 +2%p 이하에 그쳤고, SHAP에서도 평균 "
     "기여도가 약 46 GPa로 2위 변수의 약 15배에 달하였다. 즉 모델의 탄성계수 예측은 사실상 제하 강성 하나에 "
     "의해 지배된다고 볼 수 있다.")
body("[물리적 해석] 이 결과는 압입역학적으로 매우 타당하다. Oliver–Pharr 이론에 따르면 환산 탄성계수 Er은 "
     "제하 곡선 초기 기울기인 제하 강성 S와 직접 비례한다(Er = (√π/2)·S/√A, A는 접촉면적). 따라서 탄성계수와 "
     "가장 강하게 연결된 물리량이 바로 제하 강성이며, 신경망이 별도의 물리 지식 없이 데이터만으로 이 관계를 "
     "스스로 찾아냈음을 의미한다. 이는 모델이 수치를 단순 암기한 것이 아니라 물리적으로 의미 있는 패턴을 "
     "학습했다는 강력한 근거가 된다.")
body("[보조 변수 및 신뢰성] 다음으로 Slope4·Slope5(제하 구간의 기울기)와 일부 변위 변수가 부차적으로 "
     "기여하는데, 이들 역시 제하 거동·강성과 관련된 양이라는 점에서 합리적이다. 다만 이러한 변수들은 제하 "
     "강성과 물리적으로 상관관계가 있어 중요도가 Stiffness2에 집중되는 경향이 나타날 수 있다. 그럼에도 원리가 "
     "독립적인 두 기법이 동일하게 제하 강성을 1순위로 제시한 점에서, '제하 강성이 탄성계수 예측의 핵심 인자'"
     "라는 결론은 높은 신뢰성을 갖는다.")
