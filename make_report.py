...
body("[학습 과정 해석] 그림 4의 MAPE 곡선은 학습이 진행되는 동안 모델의 거동을 단계적으로 보여 준다. "
     "①(초기 급감, 약 0~5 epoch) Training MAPE는 약 63%, Validation MAPE는 약 25%에서 출발하여 불과 수 "
     "epoch 만에 각각 5%, 2% 수준으로 급격히 감소한다. 이는 신경망이 입력 변수와 탄성계수 사이의 가장 지배적인 "
     "관계(특히 제하 강성과 E의 비례 관계)를 매우 빠르게 학습함을 의미한다. ②(최적 일반화, 약 5~30 epoch) "
     "Validation MAPE가 최저(약 2%)에 도달하여 모델이 처음 보는 데이터에 대해 가장 잘 일반화되는 구간이다. "
     "이때 Validation MAPE가 Training MAPE보다 오히려 낮게 나타나는데, 이는 학습 시에만 적용되는 Dropout과 "
     "L2 정규화가 Training 오차를 다소 높이는 반면 검증 시에는 비활성화되기 때문이다. ③(과적합, 약 30 epoch "
     "이후) Training MAPE는 약 1.3%까지 계속 감소하지만 Validation MAPE는 오히려 점진적으로 증가하여 약 "
     "15%에 이른다. 두 곡선의 간격이 점점 벌어지는 이 현상이 전형적인 과적합(overfitting)으로, 모델이 학습 "
     "데이터의 세부·잡음까지 외우기 시작하면서 일반화 성능이 저하됨을 보여 준다. ④(EarlyStopping의 역할) 본 "
     "학습은 검증 오차가 가장 낮았던 시점의 가중치를 복원(restore_best_weights)하도록 설정되어, 최종 모델은 "
     "과적합된 후반 상태가 아니라 ②의 최적 일반화 모델이 선택된다. 그 결과 테스트 성능이 MAPE 약 1.2%로 우수하게 "
     "유지되었다.")
